...
`;
    ...
    function ThemePackRow() {
      const [snap, setSnap] = React.useState({ theme: active ? active.id : null, mode });
      React.useEffect(() => subscribe(() => setSnap({ theme: active ? active.id : null, mode })), []);
      ...
      const card = (t) => {
        ...
        const prev = h("span", { className: "dsh-theme-set-prev", style: { "--dt-bg": bg, "--dt-surface": surface, "--dt-accent": accent, "--dt-text2": text, "--dt-border": textDark } }, [
